...
import com.clearkeep.data.local.model.UserKeyLocal
...
class GroupRepositoryImpl @Inject constructor(
    ...
    override fun getAllRoomsAsState() = groupDAO.getRoomsAsState().map { it.map { it.toEntity() } }

    override suspend fun getAllRooms(): List<ChatGroup> = withContext(Dispatchers.IO) {
        return@withContext groupDAO.getRooms().map { it.toEntity() }
    }
    ...
    override suspend fun updateGroup(group: ChatGroup) = withContext(Dispatchers.IO) {
        groupDAO.updateGroup(group.toLocal())
    }
    ...
    override suspend fun insertGroup(group: ChatGroup): Unit = withContext(Dispatchers.IO) {
        ...
        groupDAO.insert(
            group.copy(isDeletedUserPeer = oldGroup?.isDeletedUserPeer ?: false).toLocal()
        )
    }
    ...
    override suspend fun getGroupByID(
        ...
        return@withContext if (forceRefresh) {
            ...
            Resource.success(groupDAO.getGroupById(groupId, domain, ownerId)?.toEntity())
        }
    }

    override suspend fun getGroupByID(groupId: Long, serverDomain: String): ChatGroup? =
        withContext(Dispatchers.IO) {
            return@withContext groupDAO.getGroupById(groupId, serverDomain)?.toEntity()
        }
    ...
    override fun getGroupsByDomain(
        ...
        groupDAO.getGroupsByDomain(ownerDomain, ownerClientId).map { it.map { it.toEntity() } }

    override suspend fun getAllPeerGroupByDomain(owner: Owner): List<ChatGroup> =
        withContext(Dispatchers.IO) {
            groupDAO.getPeerGroups(owner.domain, owner.clientId).map { it.toEntity() }
        }
    ...
    override suspend fun convertGroupFromResponse(
        ...
        try {
            ...
            if (response.clientKey.senderKeyId > 0 && response.groupType == "group" && !isRegisteredKey) {
                ...
                val userKey =
                    userKeyDAO.getKey(serverDomain, ownerId) ?: UserKeyLocal(
                        serverDomain,
                        ownerId,
                        "",
                        ""
